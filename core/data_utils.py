...
def to_value_frame(df: pd.DataFrame, sg_col: str, value_col: str) -> pd.DataFrame:
    ...
    부분군 컬럼과 측정값 컬럼이 같으면 ValueError 를 발생시킨다.
    pandas 버전에 무관하게 동작하도록 단순한 연산만 사용한다.
    """
    if sg_col == value_col:
        raise ValueError("부분군 컬럼과 측정값 컬럼은 서로 달라야 합니다.")
    out = df.loc[:, [sg_col, value_col]].copy()
    out.columns = [sg_col, value_col]
...
def to_count_frame(
    ...
    out = df.loc[:, [sg_col, size_col, value_col]].copy()
    out.columns = [sg_col, size_col, value_col]
